...
```bash
rm -f analysis/_png_enso_cache/seas5_*_current.npz /tmp/skill_stats_grid_detrended.nc
uv run python analysis/png_enso_slides.py --country png
uv run python analysis/png_enso_slides.py --country tls
```

then update the issuance-specific wording in the two `index.html` files and commit.
