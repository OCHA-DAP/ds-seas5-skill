...
`pages/png-enso/` holds two committed slide PNGs plus a 2-page PDF, generated by
`analysis/png_enso_slides.py` (ERA5 × Niño3.4 teleconnection; current SEAS5 forecast by
trimester). To refresh after a new SEAS5 issuance:

```bash
rm -f analysis/_png_enso_cache/seas5_png_current.npz
uv run python analysis/png_enso_slides.py
```

then update the issuance-specific wording in `pages/png-enso/index.html` and commit.
