...
`pages/enso/` is a country-selector page (dropdown; selection and language land in
`?country=ISO3&lang=fr` for sharing) over per-country slide pairs in
`pages/enso/slides/` — inline-able SVGs with selectable text plus a vector PDF, in
English and French (`{ISO3}_slide{1,2}[_fr].svg`, `{ISO3}[_fr].pdf`) — generated by
`analysis/png_enso_slides.py` for every country in the app's export. The page inlines
the SVGs (so text is selectable) and ships the DejaVu subsets in `pages/enso/fonts/`.

Slide 1 (the El Niño teleconnection) is climatological; its heavy correlation step is
cached per country in `analysis/_png_enso_cache/corr_*.npz`, so re-renders are cheap.
Slide 2 follows the app's issuance. To refresh after a new SEAS5 issuance (needs a
current `skill_stats_grid_detrended.nc`; the script asserts its vintage):
