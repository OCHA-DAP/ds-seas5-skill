...
the 0th-2nd percentile. **West Nile has received 84-99% of normal**: the wet
...
zones sit in between at roughly 65-99%.

```{python}
#| code-summary: "cumulative season curves by zone"
#| fig-cap: "Cumulative rainfall over each zone's main-season window, zone mean of district series. Gray: every year 1981-2025; dashed: median year; brown: 2026 through July (the ERA5 record's edge). The percentage is 2026 season-to-date as a share of the normal season-to-date."
...
#| fig-cap: "Observed season-to-date rainfall as % of the normal season-to-date, per JIAF district over its zone's calendar window. Filled markers are districts having their driest season-to-date since 1981."
std_rows = []
...
(top.style.hide(axis="index")
    ...
    .set_caption("Pixel-mean OND flood recurrence by district, 1998-2025. None of these "
                 "districts is in the JIAF set (see the note below the map)."))
```
...
El Niño OND in the FloodScan record plus 2025, each labeled with its
...
#| fig-cap: "OND seasonal-max SFED in every El Niño OND of the record plus 2025, against the same color scale. Panel labels give the observed OND-mean ENSO (ONI) and IOD (DMI) indices, the flooded share of pixels (SFED >= 0.05), and its rank-based return period among the 28 seasons."
...
FACET_YEARS = [2002, 2006, 2009, 2015, 2018, 2019, 2023, 2025]
fig, axes = plt.subplots(2, 4, figsize=(13, 8.2), layout="constrained")
...
when a positive IOD co-occurs. Descriptively, the four El Niño + IOD seasons
(2006, 2018, 2019, 2023) averaged 4.9% flooded (median 3.8%) against 3.2%
(median 2.9%) in the five El Niño-only seasons. The split is not clean:
the El Niño-only group contains 2015 (5.9%), and the biggest floods of all
(2024, 2025) sit in the neither-driver group. Read it as a lean, not a rule.
...
the recurrent Kyoga/wetland core (2019: 40%). The 2023-25 floods spread far
...
(tt.style.hide(axis="index")
   ...
   .set_caption("HISTORICAL flooding, not the 2026 outlook: share of each district's area "
...
all four years issued at the 83rd percentile or above flooded above the
median season: 2015 (96th), 2006 (91st; Teso floods, with Kumi and Butaleja
among eight districts affected in EM-DAT), 2023 (89th; the largest
El Niño-year extent, though EM-DAT records no OND event that season), and
2019 (83rd; the deadliest OND in the record, with 104 deaths across the
Rwenzori and Elgon events). The counterexamples run the other way. 2018 (33rd) was not forecast wet,
...
flash-flood and landslide impacts EM-DAT says actually kill, and re-check at
