...
is exposed, ending in a priority readout.
:::
...
### The wet season's other face: where OND water becomes flooding
...
Read with the indices, the record splits cleanly. El Niño **with** a
positive IOD (2006, 2018, 2019, 2023) or with extreme strength (2015)
floods well above the median season; El Niño **without** the IOD (2002,
2009) barely doubles a normal year. And the biggest flood of all, 2025,
had *negative* indices: it rode the wet antecedent state left by 2023-24
(high Kyoga and Victoria stands, saturated wetlands). Quantitatively, over
the 28 seasons: the ocean drivers strongly set the season's *rainfall*
(Spearman r of OND rain with DMI +0.63, with ONI +0.55), but rainfall alone
only weakly sets the *flooded area* (r +0.28), and last season's flooded
area is nearly as informative (lag-1 r +0.38). Flood extent in Uganda is
rainfall plus memory: what the lakes and wetlands are already holding.

```{python}
#| code-summary: "rainfall vs flooded area, with drivers"
#| fig-cap: "Each point is one OND season: national mean OND rainfall (x) against flooded area (y, log scale). Fill color: the observed IOD (DMI). Ringed points followed a top-5 flood season, the memory effect. Small-sample honesty: n=28, and only the rain-driver link is statistically firm."
...
2019, 2023) shows *where* a wet season changes the map, in return-period
terms: in the Teso / Kyoga core the flood RP tightens from about one year in
...
Both levers point the same way this year. The ocean drivers are aligned:
...
enter this season high. Neither guarantees a flood year (n=28, and the
rain-to-flood link is the weak step in the chain), but the historically
