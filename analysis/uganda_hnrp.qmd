...
Forecast × HNRP pipeline deliberately excludes as not the country's own plan —
so it does not appear in the app. This standalone analysis reproduces the
**Forecast × HNRP tab's methodology** for Uganda using supplementary data from
the country team:
...
- **Targeted and reached**: *Gender-MAX Targeted / Achieved People*
  (2026-08-19), district-level maxima with gender and disability breakdowns.
- **Climate**: the same SEAS5 / ERA5 sources the app uses. Region level comes
  ...
  registry), with exactextract over the same COGs — validated to reproduce the
  DB's region means within ~1% — then run through the repo's unchanged
  skill/return-period code.
:::
...
The table and map below show, per unit, the **worst qualifying drought slot**
at the latest SEAS5 issuance — exactly the app's rule: valid trimesters
(lead −2…4), rainy-season only, skill $r \ge 0.30$ (detrended), dry-side
forecasts only (percentile < 50), severity = the drought return period. The RP
says how *unusual* the forecast is, never how much water is missing, so each RP
is paired with the forecast and climatological-normal seasonal totals (mm).
...
#| fig-cap: "Worst qualifying drought slot per district, latest SEAS5 issuance. JIAF 2.0 Light districts outlined in black; the app's category palette."
...
ax.legend(handles=[
    ...
    Patch(facecolor="none", edgecolor="#1d2021", label="JIAF 2.0 Light district"),
...
::: {.callout-important}
## Dry now, wet later — read the timing

The drought signal is concentrated in **June–September 2026**: June was the
...
percentile. From **October onward the same issuance forecasts wetter than
normal** (Oct 91st / Nov 96th / Dec 98th percentile for Northern region) — so a
"wetter than normal OND" reading elsewhere in the app is correct and does not
...
Jun–Jul–Aug slots show $r$ 0.92–0.96 because they are in-season at an August
issuance — June and July are already *observed*, so two-thirds of that signal
is monitoring, not forecast. The fully-forecast Aug–Sep–Oct slots carry genuine
forecast skill of $r$ 0.34–0.62 (median 0.57); each table row reports its own
$r$.

**Crop relevance:** the Jun–Aug window carries **38–44% of total annual
rainfall in every Karamoja district** (ERA5 climatology, this analysis). What
that means for production differs by agro-ecological zone — see
...
The empirical RP is capped at 46 years (the 45-year record + 1); at this
...
is; the map below shows how **deep** the deficit is — the forecast seasonal
total as a share of the climatological normal, which is where districts differ.
:::
...
Cropping seasons come from the **JRC ASAP sub-national crop calendars**
(planting / growth / harvest windows per agro-ecological zone, in 10-day
dekads), with each CODAB district assigned to its zone
(`pipeline/build_uga_crop_calendar.py`). Three structurally different
situations emerge:
...
- **Karamoja is pastoral, not a cropping zone**: ASAP publishes *no crop
  ...
  record-dry Jun–Aug (its rainfall peak) is a **pasture, water and livestock**
  crisis first, hitting the sparse sorghum patches on top. The wet Oct–Dec can
  recharge water and pasture, but Karamoja's rains normally *end* around
  October — an unusual wet OND is recovery for herds, not for a harvest.
- **West Nile is unimodal in ASAP** (no second-season crops): planting
  March–April, harvest June–July. The record-dry Jun–Sep lands on
  grain-fill/harvest of the *only* season, and there is **no second planting
  until March 2027** — the wet OND forecast does not offer a cropping recovery
  in exactly the districts hosting the refugee response (Yumbe, Adjumani,
  Madi-Okollo & Terego, Obongi, Koboko, Arua/Terego, Moyo).
- **The bimodal zones** (Mid Northern incl. Lamwo; Lake Albert Crescent incl.
  ...
  Drylands incl. Isingiro; Lake Victoria Crescent incl. Kampala): main-season
  harvest (Jun–Aug) is hit by the dry window, and **second-season planting
...
#| fig-cap: "Maize seasons per ASAP agro-ecological zone (representative of beans/millet, which differ by ≤ a dekad or two). Brown band: the record-dry Jun–Sep window; green band: the wetter-than-normal Oct–Dec forecast. Karamoja has no ASAP crop calendar — it is 91% rangeland."
...
(2026-08-19). **Each admin level is an independent rollup, not a sum of the
finer level** — district figures here are the district-level series; never add
levels together.
...
- **Forecast selection** is the app's rule verbatim (code imported from
  ...
  Forecast × HNRP tab.
- **District climate stats are new**: Uganda sits outside the rasterstats DB's
  ...
  `pipeline/compute_skill_uga_adm2.py` ran the standard skill code. Region-level
  exactextract means reproduce the DB's stored means within ~1%.
...
- **The empirical return period saturates at 46 years** (record length + 1);
  a district shown at 46 is "drier than anything since 1981", not "exactly a
  46-year event". Use the deficit (% of normal) to compare districts once the
  cap binds.
- **Terego** (created 2020) is not in the CODAB vintage: its rows keep their
  own people figures but inherit UG3072 (Arua)'s forecast; "Madi-Okollo &
  Terego" inherits UG3084 (Madi Okollo).
